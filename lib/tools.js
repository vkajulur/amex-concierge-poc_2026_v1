...
const toolSchemas = [
  ...
  { name: "request_credit_increase",
    description: "Request a credit limit increase. Optionally include the requested new limit.",
    input_schema: { type: "object", properties: {
      amount: { type: "number", description: "Requested new credit limit; optional." } }, required: [] } },
...
const executors = {
  ...
  check_card_upgrade() {
    const u = load("profile.json").upgrade;
    if (!u.eligible) return { result: "Not currently pre-qualified for an upgrade.", ui: { type: "card_upgrade", eligible: false } };
    return { result: `Eligible to upgrade to ${u.offeredCard}.`, ui: { type: "card_upgrade", eligible: true, ...u } };
  },
  ...
  request_credit_increase({ amount }) {
    const a = load("profile.json").account;
    const lines = amount != null
      ? [`Current limit: ${money0(a.creditLimit)}`, `Requested limit: ${money0(Number(amount))}`, "Soft pull \u2014 won't affect your score"]
      : [`Current limit: ${money0(a.creditLimit)}`, "We'll review your account for an increase", "Soft pull \u2014 won't affect your score"];
    return { result: "Prepared a credit limit increase request for confirmation.",
      ui: confirmCard("Request credit limit increase", lines, "Submit request", "Request submitted. We'll notify you with a decision shortly.") };
  },

  update_contact({ field, value }) {
    const label = field === "mobile" ? "mobile number" : "email address";
    return { result: `Prepared ${label} update to ${value} for confirmation.`,
      ui: confirmCard(`Update ${label}`, [`New ${label}: ${value}`, "A verification code will be sent"], "Update", `Your ${label} has been updated to ${value}.`) };
  },
